...
from back_end import heloMother
...
def Search_1(input_query):
    ...
    last_pos = df.tail(1).index[0]
    df_result = search_engine(df,k=20, last_pos=last_pos)
...
def Search_2(input_query):
    ...
    df = append_new_query_to_tail_df2(input_query)

    df_result = heloMother(df)
